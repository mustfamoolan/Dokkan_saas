...
$ids1 = collect($res1->items())->pluck('id')->toArray();
echo "SQL: " . DB::getQueryLog()[0]['query'] . "\n";
echo "Bindings: " . json_encode(DB::getQueryLog()[0]['bindings']) . "\n";
echo "Total in Paginator: " . $res1->total() . "\n";
echo "IDs found: [" . implode(',', $ids1) . "]\n";

DB::flushQueryLog();

echo "\n--- VALIDATING CATEGORY 66 (SUBCAT) ---\n";
$res66 = $productService->getAllProducts(['category_id' => 66]);
$ids66 = collect($res66->items())->pluck('id')->toArray();
echo "SQL: " . DB::getQueryLog()[0]['query'] . "\n";
echo "Bindings: " . json_encode(DB::getQueryLog()[0]['bindings']) . "\n";
echo "Total in Paginator: " . $res66->total() . "\n";
echo "IDs found: [" . implode(',', $ids66) . "]\n";
