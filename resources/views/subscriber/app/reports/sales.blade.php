    ...
    <div class="col-md-6">
        <div class="card border-0 shadow-sm bg-soft-success">
            <div class="card-body py-4">
                <div class="text-muted small mb-1">إجمالي المبيعات (الصافي)</div>
                <h2 class="fw-bold text-success mb-0">{{ number_format($data['total_amount'], 2) }} <small>د.ع</small></h2>
                <div class="mt-2 small">{{ $data['invoice_count'] }} فاتورة بيع معتمدة</div>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-6">
        <div class="card border-0 shadow-sm h-100">
            <div class="card-header bg-white py-3">
                <h5 class="mb-0 fs-16">أفضل 5 عملاء شراءً</h5>
            </div>
            <div class="card-body p-0">
                <div class="list-group list-group-flush">
                    @forelse($data['top_customers'] as $item)
                    <div class="list-group-item d-flex justify-content-between align-items-center py-2 px-3">
                        <span class="small fw-bold">{{ $item->customer->name }}</span>
                        <span class="badge bg-soft-primary text-primary">{{ number_format($item->total, 2) }}</span>
                    </div>
                    @empty
...
<div class="card border-0 shadow-sm">
    <div class="card-header bg-white py-3">
        <h5 class="mb-0 fs-16">سجل فواتير البيع للفترة المحدرة</h5>
    </div>
    <div class="card-body p-0">
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="small">{{ $invoice->customer->name }}</td>
                        <td class="text-end pe-3 fw-bold">{{ number_format($invoice->total_amount, 2) }}</td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="4" class="text-center py-5 text-muted">لا توجد فواتير بيع في هذه الفترة</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
